...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        #t_bins = self.create_t_bins(num_sample, ray_bundle.origins.device)  #[0.0, 1.0]
        t_bins = self.create_t_bins(num_sample+1, ray_bundle.origins.device)[:-1]  #[0.0, 0.50]
        t_bins = t_bins.unsqueeze(0).repeat(ray_bundle.origins.shape[0], 1)
        sample_size = 1.0 / num_sample
        t_bins = t_bins + torch.rand_like(t_bins) * sample_size
        float_near = ray_bundle.nears[0].item()
        float_far = ray_bundle.fars[0].item()
        t_samples = self.map_t_to_euclidean(t_bins, float_near, float_far)
