...
def step(points, pc_labels, class_labels, model):
    ...
    # TODO : Implement step function for segmentation. -- DONE
    points = points.to(device)
    class_labels = class_labels.to(device)
    pc_labels = pc_labels.to(device)
    logits, inp_trans, feat_trans = model(points)
    loss = F.cross_entropy(logits, pc_labels)
    orthogonal = get_orthogonal_loss(feat_trans)
    loss = loss + orthogonal
...
def train_step(points, pc_labels, class_labels, model, optimizer, train_acc_metric):
    ...
    # TODO : Implement backpropagation using optimizer and loss -- DONE
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
...
def main(args):
    ...
    # TODO: Implement the model call -- DONE
    model = PointNetPartSeg(input_transform=True, feature_transform=True, num_parts=50)
